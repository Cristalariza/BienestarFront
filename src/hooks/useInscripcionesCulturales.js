...
import { inscripcionesCulturalesService, personasService } from "../services";

export const useInscripcionesCulturales = () => {
  ...
  const cargarInscripciones = async () => {
    try {
      setLoading(true);
      console.log('Cargando inscripciones culturales...');

      // Cargar inscripciones
      const inscripcionesData = await inscripcionesCulturalesService.obtenerTodas({
        skip: 0,
        limit: 1000
      });
      ...
      if (error.status !== 503) {
        toast.current?.show({
          severity: 'error',
          summary: 'Error',
          detail: 'Error al cargar las inscripciones culturales',
          life: 3000
        });
      }
      setInscripciones([]);
  ...
  const handleEstadoChange = async (inscripcionId, nuevoEstado) => {
    try {
      ...
      await inscripcionesCulturalesService.cambiarEstado(inscripcionId, nuevoEstado);
  ...
  const getFilteredInscripciones = () => {
    ...
    return resultado.filter((inscripcion) => {
      ...
      const programa = inscripcion.programa_cultural?.nombre || '';
